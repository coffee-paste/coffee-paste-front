        ...
        <Card>
          ...
          <template #footer>
            <div
              ...
              class="login-provider-container"
            >
              <div class="login-provider-container">
                <Button
                  ...
                  type="button"
                  label="Primary"
                  class="p-button-outlined p-button-lg login-button"
                >
                  ...
                  <span class="p-ml-2 p-text-bold login-provider-text"
...
const providers = [
  {
    ...
    name: "Google",
    icon: googleImage,
    oauthUrl: `https://accounts.google.com/o/oauth2/v2/auth?client_id=${VUE_APP_GOOGLE_CLIENT_ID}&response_type=code&scope=https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/userinfo.email&redirect_uri=${redirectUri}`,
  },
  {
    provider: "GITHUB",
    name: "GitHub",
    icon: githubImage,
    oauthUrl: `https://github.com/login/oauth/authorize?client_id=${VUE_APP_GITHUB_CLIENT_ID}`,
  },
  {
    provider: "GITLAB",
    name: "GitLab",
    icon: gitlabImage,
    oauthUrl: `https://gitlab.com/oauth/authorize?client_id=${VUE_APP_GITLAB_CLIENT_ID}&redirect_uri=${redirectUri}&response_type=code&state=STATE&scope=read_user`,
  },
];
...
export default {
  ...
  methods: {
    login(provider) {
      localStorage.setItem("login-with", provider.provider);
      window.location.href = provider.oauthUrl;
    },
    async auth(code) {
      const providerName = localStorage.getItem("login-with");
      localStorage.setItem("login-with", "");
      ...
      try {
        const options = {
          ...
          body: JSON.stringify({
            code,
            redirectUri,
            oauth2Service: provider.provider,
          }),
          headers: {
        ...
        const workspaceResponse = await fetch(
          `${VUE_APP_API_URL}/notes/workspace`,
          o1
        );
        ...
        if (workspace.length < 1) {
          ...
          await fetch(`${VUE_APP_API_URL}/notes`, o1);
        }
        window.location.href = `${redirectUri}/#/workspace`;
      } catch (error) {
        console.log(error);
