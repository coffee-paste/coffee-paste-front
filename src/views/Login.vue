                            ...
                            <div v-else>
                                ...
                                >
                                    <div class="login-provider-container">
                                        ...
                                        >
                                            <div
                                                class="login-provider-icon-container"
                                            >
                                                <img
                                                    alt="logo"
                                                    :src="provider.icon"
                                            ...
                                            <span
                                                class="p-ml-2 p-text-bold login-provider-text"
                                                >Login with
                                                {{ provider.name }}</span
                                            >
                                        </Button>
...
const providers: OAuthProvider[] = [
    {
        ...
        displayName: "Google",
        icon: googleImage,
        oauthUrl: `https://accounts.google.com/o/oauth2/v2/auth?client_id=${envFacade.googleClientId}&response_type=code&scope=https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/userinfo.email&redirect_uri=${redirectUri}`,
    },
    {
        name: "GITHUB",
        displayName: "GitHub",
    ...
    },
    {
        ...
        displayName: "GitLab",
        icon: gitlabImage,
        oauthUrl: `https://gitlab.com/oauth/authorize?client_id=${envFacade.gitlabClientId}&redirect_uri=${redirectUri}&response_type=code&state=STATE&scope=read_user`,
    },
];
...
export default defineComponent({
    ...
    methods: {
        ...
        async auth(code: string) {
            ...
            try {
                const authResponse = await new AuthenticationApi().authByOAuth({
                    code,
                    redirectUri,
                ...
                const profile = await new UsersApi({
                ...
                const workspace = await new NotesApi({
                    apiKey: credentialsManager.getToken(),
                }).getOpenNotes();
