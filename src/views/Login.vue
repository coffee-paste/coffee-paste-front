								...
								<div v-for="provider of providers" :key="provider.name">
									<div class="login-provider-container">
										<Button @click="login(provider)" type="button" label="Primary" class="p-button-outlined p-button-lg login-button">
											...
											<span class="p-ml-2 p-text-bold login-provider-text">Login with {{ provider.name }}</span>
										</Button>
									</div>
								</div>
							</div>
...
export default defineComponent({
	...
	methods: {
		login(provider: OAuthProvider) {
			...
			setLocalStorageItem<string>(LocalStorageKey.LoginWith, provider.name, {
				itemType: 'string',
			});
		...
		async auth(code: string) {
			...
			try {
				...
				if (envFacade.isDevMode) {
					credentialsManager.setToken(authResponse.headers.get('Authentication') || '');
				}
